...
static int wlan_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if(NULL == msg_q)
	{
		...
		if( ( msg_num(&(vif->msg_q[q_index])) >= (vif->msg_q[q_index].num * 39 / 40) ) && (false == vif->flow_ctrl_set) )
		{
...
int marlin_priv_cmd(struct net_device *ndev, struct ifreq *ifr)
{
	...
	} else if (strnicmp(command, CMD_AUTO_CHANNEL,
					strlen(CMD_AUTO_CHANNEL))==0) {
		printke("AUTO CHANNEL SELECTION!\n");
		vif->auto_channel = 1;
	} else {
		...
		for(vif_id = NETIF_0_ID; vif_id < WLAN_MAX_ID; vif_id++)
		{
			...
			qos_sched(&(vif->qos), &msg_q, &need_tx);
				...
				if( true == vif->flow_ctrl_set )
				{
					...
					{
						if (msg_num(&(vif->msg_q[i])) > (vif->msg_q[i].num * 6 / 10))
							break;
					}
					if (i == MSG_Q_MAX_ID)
						wake_net(vif->id);
...
static int wlan_tx_buf_alloc(wlan_vif_t *vif)
{
	...
	msg_q_alloc(&(vif->msg_q[MSG_Q_ID_0]), sizeof(tx_msg_t), 300);
